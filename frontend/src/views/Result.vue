        ...
        <a-affix :offset-top="80">
          <a-menu mode="inline" :selected-keys="[activeSection]" @click="scrollToSection">
            ...
            <a-sub-menu key="days" title="📅 每日行程">
              <a-menu-item v-for="(_day, dayIndex) in tripPlan.days" :key="`day-${dayIndex}`">
                第{{ dayIndex + 1 }}天
              </a-menu-item>
            </a-sub-menu>
...
const getMapAttractions = () => {
  ...
  return tripPlan.value.days.flatMap((day, dayIndex) =>
    day.attractions
      ...
      .map((attraction, attrIndex) => ({
        name: attraction.name,
        longitude: Number(attraction.location.longitude),
        latitude: Number(attraction.location.latitude),
        dayIndex,
        attrIndex
      }))
  )
...
const getStaticMapZoom = (lngRange: number, latRange: number) => {
  const maxRange = Math.max(lngRange, latRange)
  if (maxRange > 1) return 9
  if (maxRange > 0.5) return 10
  if (maxRange > 0.25) return 11
  if (maxRange > 0.12) return 12
  if (maxRange > 0.06) return 13
  return 14
}

const createStaticMapUrls = () => {
  const attractions = getMapAttractions()

  if (attractions.length === 0) {
    return []
  }

  const longitudes = attractions.map(item => item.longitude)
  const latitudes = attractions.map(item => item.latitude)
  ...
  const markers = attractions
    .map((item, index) => `mid,0x34c759,${index + 1}:${item.longitude},${item.latitude}`)
    .join('|')
  const paths = tripPlan.value?.days
    .map((_, dayIndex) => attractions.filter(item => item.dayIndex === dayIndex))
    .filter(dayAttractions => dayAttractions.length > 1)
    .map(dayAttractions => `5,0x1890ff,0.8,,:${dayAttractions.map(item => `${item.longitude},${item.latitude}`).join(';')}`)
    .join('|') || ''
  const params = new URLSearchParams({
    location: `${centerLng},${centerLat}`,
    zoom: String(getStaticMapZoom(lngRange, latRange)),
    size: '1000*560',
    scale: '2',
    markers
  })
  ...
  const urls = [`${API_BASE_URL}/api/map/static?${params.toString()}`]
  const amapKey = import.meta.env.VITE_AMAP_WEB_JS_KEY
  if (amapKey) {
    const directParams = new URLSearchParams(params)
    directParams.set('key', amapKey)
    urls.push(`https://restapi.amap.com/v3/staticmap?${directParams.toString()}`)
  }

  return urls
}

const waitForImageLoad = (img: HTMLImageElement, url: string) =>
  new Promise<void>((resolve, reject) => {
    img.onload = () => resolve()
    img.onerror = () => reject(new Error(`静态地图加载失败: ${url}`))
  })

const loadFirstAvailableMapImage = async (urls: string[]) => {
  ...
  throw lastError || new Error('静态地图加载失败')
}

const replaceExportMap = async (exportContainer: HTMLElement) => {
  ...
  const mapHeight = liveMapContainer?.offsetHeight || 500
  exportMapContainer.style.height = `${mapHeight}px`
  exportMapContainer.style.background = '#f5f7fa'

  const staticMapUrls = createStaticMapUrls()
  if (staticMapUrls.length === 0) {
    exportMapContainer.innerHTML = '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#8c8c8c;background:#f5f7fa;">暂无可导出的地图数据</div>'
    return
  }

  const img = await loadFirstAvailableMapImage(staticMapUrls)
  exportMapContainer.innerHTML = ''
  exportMapContainer.appendChild(img)
}
...
const exportAsImage = async () => {
  try {
    ...
    await replaceExportMap(exportContainer)
